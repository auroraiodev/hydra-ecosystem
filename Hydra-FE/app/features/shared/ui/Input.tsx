...
function InputIcon({ icon }: { icon: InputProps['icon'] }) {
  if (!icon) return null;
...
export const Input = ({
  ...
  const inputType =
    isPassword && showPasswordToggle ? (isPasswordVisible ? 'text' : 'password') : type;

  return (
    <div className="flex flex-col gap-1.5 w-full">
      {label && <label className="text-sm font-black text-text-muted px-1">{label}</label>}
