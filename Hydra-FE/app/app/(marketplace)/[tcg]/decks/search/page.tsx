...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Decks Preconstruidos de ${tcgName} | Hydra Collectables`,
    description: `Encuentra los mejores decks preconstruidos de ${tcgName} en México.`,
    alternates: { canonical: `/${tcg}/decks/search` },
  };
}

export default async function TcgDecksSearchPage({ params, searchParams }: Props) {
  ...
  if (!resolvedTcgId && tcg) {
    try {
      ...
      console.error('[DecksSearch] Error resolving TCG ID:', err);
    }
  }
  ...
  return (
    <Suspense fallback={<SearchSkeleton />}>
      <SearchDataLoader category="PRECON_DECK" tcgId={resolvedTcgId} isLocal={true} page={page} />
    </Suspense>
  );
}
