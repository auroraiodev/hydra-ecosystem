...
export class MaintenanceGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Try to get role from request.user (set by JwtAuthGuard) or manual JWT verify
    let userRole: string | undefined;

    // Check request.user first (if JwtAuthGuard ran before this)
    if (request.user && request.user.role) {
      userRole = request.user.role.name;
    }

    // Fallback: Manual JWT verification if request.user is not yet set (e.g. public routes)
    if (!userRole) {
      ...
      if (token) {
        try {
          ...
          // Attach basic user info to request if not present
          if (!request.user) {
            request.user = {
              id: payload.sub,
              email: payload.email,
