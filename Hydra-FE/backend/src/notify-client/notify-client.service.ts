...
export class NotifyClientService implements OnModuleInit, OnModuleDestroy {
  ...
  // SSE stream subjects, keyed by userId — kept here since hydra-be owns HTTP connections
  ...
  onModuleInit() {
    ...
    this.subscriber
      .connect()
      .then(() => {
        // Subscribe to all per-user notification channels published by hydra-notify
  ...
  getStream(userId: string) {
    ...
    if (!this.streams.has(userId)) {
      ...
      this.logger.debug(
        `Stream created for user ${userId} (ref count: ${this.streamRefCount.get(userId)})`,
      );
    }
  ...
  closeStream(userId: string) {
    ...
    if (count <= 0) {
      ...
      if (subject) {
        ...
        this.logger.debug(`Stream closed and removed for user ${userId}`);
      }
    } else {
      this.streamRefCount.set(userId, count);
      this.logger.debug(`Stream ref count decremented for user ${userId} (remaining: ${count})`);
    }
  }
