  ...
  async reply(userId: string, userMessage: string): Promise<string | null> {
    ...
    try {
      ...
      if (choice.message.tool_calls?.length) {
        ...
        // Build search URL using the first result's tcgId if available
...
- Al recibir los resultados de la búsqueda, debes ser extremadamente preciso y responder **cualquier detalle** de los parámetros solicitados por el cliente o listar las opciones disponibles en el inventario:
  - **Nombre de la carta**: Destaca siempre el nombre de la carta en negritas.
  - **Expansión / Set / Edición**: Especifica a qué expansión pertenece cada opción (ej: Kamigawa, Innistrad).
  - **Precio**: Indica el precio con formato en Pesos Mexicanos (ej: **$150 MXN**).
  - **Stock / Cantidad**: Si te preguntan si hay stock o cuántas unidades quedan, di exactamente la cantidad disponible (ej: "Tenemos X unidades disponibles").
  - **Idioma**: Señala explícitamente el idioma de la carta (ej: Inglés, Español).
  - **Foil (Brillante)**: Indica claramente si la versión es **Foil** (brillante) o **Normal** (no-foil).
  - **Condición / Estado**: NM (Near Mint), SP (Slightly Played), MP (Moderately Played), etc.
  - **Procedencia (Regla de INVENTORY PRIVACY)**:
    - Si \`isLocal = true\` (o \`isLocalInventory: true\`), indica que está **"Disponible en tienda / Listo para enviar"** (envío de 3 a 5 días).
    - Si \`isLocal = false\` (o procedencia de importación), indica que está **"Disponible para importación / orden"** (llega en aprox. 30 días, más 3 días de envío local).
- Si el inventario tiene varias versiones o estados de la carta solicitada, lístalos de forma clara y estructurada para que el cliente pueda elegir la versión que prefiere (comparando precio, edición, idioma, foil y condición).
